...
# For backward compatibility with existing code, create type-specific configs
class ADXProviderConfig(BaseModel):
    """ADX-specific provider config (generated from ProviderConfig)."""
...
class SQLProviderConfig(BaseModel):
    """SQL-specific provider config (generated from ProviderConfig)."""

    id: str
    type: str = "sql"
    target: SQLResourceConfig
    default_timeout_seconds: float = 30.0
    retry_attempts: int = 3
    credentials: Optional[str] = None
    metadata: Dict[str, Any] = Field(default_factory=dict)
...
class RESTProviderConfig(BaseModel):
    """REST-specific provider config (generated from ProviderConfig)."""
...
class CSVProviderConfig(BaseModel):
    """CSV-specific provider config (generated from ProviderConfig)."""
