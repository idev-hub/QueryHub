...
def run_report(
    ...
    output_html: Optional[Path] = typer.Option(
        None,
        help="Optional path to write the rendered HTML"
    ),
    ...
    try:
        result = asyncio.run(
            _run_report(report_id, config_dir, templates_dir, output_html, email)
        )
...
async def _run_report(
    ...
    try:
        ...
        typer.echo(
            f"Report '{result.report.title}' generated at {result.generated_at.isoformat()}"
        )
